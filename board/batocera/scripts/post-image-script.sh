...
then
    ...
    # when there are several one, the first one is the common directory where to find the post-image-script.sh directory
...
BATOCERA_POST_IMAGE_SCRIPT="${BR2_EXTERNAL_BATOCERA_PATH}/board/batocera/${COMMON_PARENT_DIR}/post-image-script.sh"
...
do
    ...
    if "${BATOCERA_LOWER_TARGET}" != "${BATOCERA_SUBTARGET}"
    then
    ...
    genimage --rootpath="${TARGET_DIR}" --inputpath="${BATOCERA_BINARIES_DIR}/boot" --outputpath="${BATOCERA_BINARIES_DIR}" --config="${GENIMAGEFILE}" --tmppath="${GENIMAGE_TMP}" || exit 1
